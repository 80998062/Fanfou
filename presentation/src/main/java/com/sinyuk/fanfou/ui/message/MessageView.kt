...
class MessageView : AbstractLazyFragment(), Injectable {
    ...
    override fun lazyDo() {
        accountViewModel = obtainViewModel(factory, AccountViewModel::class.java).apply {
            accountRelay.observe(this@MessageView, Observer<String> {
                adminLive?.removeObserver(adminOB)
                adminLive = admin(it).apply { observe(this@MessageView, adminOB) }
            })
        }
    }
    ...
    private val adminOB: Observer<Player> = Observer { t ->
        t?.let {
            Log.d(MessageView::class.java.simpleName,t.uniqueId)
        }
    }
}
