...
class RemoteDataSource constructor(application: Application, endpoint: Endpoint, interceptor: Oauth1SigningInterceptor) : RemoteTasks {
    override fun fetchFavorites(uniqueId: String?, since: String?, max: String?): Single<MutableList<Status>> =
            restAPI.fetch_favorites(uniqueId, since, max).map(ErrorCheckFunction(gson))

    override fun fetchFromPath(path: String, since: String?, max: String?): Single<MutableList<Status>> =
            restAPI.fetch_from_path(path, since, max).map(ErrorCheckFunction(gson))
