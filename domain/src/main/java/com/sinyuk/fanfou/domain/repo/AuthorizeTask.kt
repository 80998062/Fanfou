 ...
 */
class AuthorizeTask constructor(val uniqueId: String?,
                                val account: String,
                                val password: String,
                                private val client: OkHttpClient,
                                @Named(DATABASE_IN_DISK) val db: LocalDatabase,
                                @Named(TYPE_GLOBAL) val preferences: SharedPreferences) : Runnable {

    val liveData: MutableLiveData<Resource<Authorization>> = MutableLiveData()

    @SuppressLint("CommitPrefEdits")
    override fun run() {
        val current = db.playerDao().query(uniqueId)
        if (current.value?.authorization == null) {
            try {
            ...
            liveData.postValue(Resource.success(current.value!!.authorization))
        }
    }
}