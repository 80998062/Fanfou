 ...
 *
 */
class StatusDataSourceFactory(private val db: LocalDatabase, private val dao: StatusDao, private val path: Int) : DataSource.Factory<String, Status> {
    val sourceLiveData = MutableLiveData<StatusDataSource>()

    override fun create(): DataSource<String, Status> {
        val source = StatusDataSource(db, dao, path)
